<template>
  <v-dialog v-model="deleteDialog" max-width="290" @click:outside="closeDialog">
    <v-card :loading="loading">
      ...
      <v-card-text>Είστε σίγουροι ότι θέλετε να διαγράψετε την επίσκεψη στον {{visit.customer ? visit.customer.name : ''}}?</v-card-text>
...
export default {
  ...
  methods: {
    ...
    deleteItem() {
      this.loading = true;
      axios.delete(`visits/${this.visit.id}`).then(res => {
        this.$emit("closeDialog");
        this.$emit("visitDeleted");
        this.loading = false;
      });
    }
  }
